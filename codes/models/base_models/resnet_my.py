...
def resnet18_v1b(pretrained=False, pretrained_base='',local_rank=None, **kwargs):
    model = ResNetV1b(BasicBlockV1b, [2, 2, 2, 2], **kwargs)
    if pretrained != 'None':
        if local_rank is not None:
            old_dict = torch.load(pretrained_base, map_location=torch.device(local_rank))
        else:
            old_dict = torch.load(pretrained_base)
