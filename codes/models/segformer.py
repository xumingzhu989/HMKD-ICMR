...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        att=self.attn(self.norm1(x), H, W)#B,H*W,C
        #print('Attention_shape\t{}'.format(att.shape))
...
class Segformer(nn.Module):
    ...
    '''
    [512,512]
    teacher begin:tf1,tf2,tf3,tf4
    t-cnn_c1.shape  torch.Size([4, 64, 128, 128])
    t-cnn_c2.shape  torch.Size([4, 128, 64, 64])
    t-cnn_c3.shape  torch.Size([4, 320, 32, 32])
    t-cnn_c4.shape  torch.Size([4, 512, 16, 16])
    teacher over:tf1,tf2,tf3,tf4
    '''
    '''
    teacher begin:tf1,tf2,tf3,tf4
    t-cnn_c1.shape  torch.Size([4, 64, 128, 256])
    t-cnn_c2.shape  torch.Size([4, 128, 64, 128])
    t-cnn_c3.shape  torch.Size([4, 320, 32, 64])
    t-cnn_c4.shape  torch.Size([4, 512, 16, 32])
    teacher over:tf1,tf2,tf3,tf4
    '''
    def forward_features(self, x):
        B = x.shape[0]
        outs = []
        # print('teacher begin:tf1,tf2,tf3,tf4')
        # stage 1
        ...
        for i, blk in enumerate(self.block1):
            x,att1 = blk(x, H, W)
            #att1=torch.mean(att1,dim=2).unsqueeze(2)
            att_map_1.append(att1)

        x = self.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # print('t-cnn_c1.shape\t{}'.format(x.shape))
        outs.append(x)
        ...
        for i, blk in enumerate(self.block2):
            x,att2 = blk(x, H, W)
            #att2 = torch.mean(att2, dim=2).unsqueeze(2)
            att_map_2.append(att2)
        x = self.norm2(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # print('t-cnn_c2.shape\t{}'.format(x.shape))
        outs.append(x)
        ...
        for i, blk in enumerate(self.block3):
            x,att3 = blk(x, H, W)
            #att3 = torch.mean(att3, dim=2).unsqueeze(2)
            att_map_3.append(att3)
        x = self.norm3(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # print('t-cnn_c3.shape\t{}'.format(x.shape))
        outs.append(x)
        ...
        for i, blk in enumerate(self.block4):
            x,att4 = blk(x, H, W)
            #att4 = torch.mean(att4, dim=2).unsqueeze(2)
            att_map_4.append(att4)#有3个
            #print(att4.shape)#torch.Size([1, 256, 512])#B HW C
            #print(att4)
        #print('MiT_B4 stage4 att_map len\t{}\tatt_map[0] shape\t{}'.format(len(att_map_4),att_map_4[0].shape))
        x = self.norm4(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # print('t-cnn_c4.shape\t{}'.format(x.shape))
        outs.append(x)
        # print('teacher over:tf1,tf2,tf3,tf4')
        # 返回通道平均之后的注意力图
        return outs,att_map_1,att_map_2,att_map_3,att_map_4

    def forward(self, x):
        x,att_map_1,att_map_2,att_map_3,att_map_4= self.forward_features(x)
        ...
        # print('_c.shape\t{}'.format(_c.shape))
        # # torch.save(_c, '_c.pt')
        # # #return [x, _c,att_map_1,att_map_4]
        # for i in range(len(att_map_1)):
        #     print('att_map_1_{}.shape\t{}'.format(i,att_map_1[i].shape))
        #     #torch.save(att_map_1[i], f'attmap1_{i}.pt')
        # for i in range(len(att_map_2)):
        #     print('att_map_2_{}.shape\t{}'.format(i,att_map_2[i].shape))
        #     #torch.save(att_map_2[i], f'attmap2_{i}.pt')
        # for i in range(len(att_map_3)):
        #     print('att_map_3_{}.shape\t{}'.format(i,att_map_3[i].shape))
        #     #torch.save(att_map_3[i], f'attmap3_{i}.pt')
        # for i in range(len(att_map_4)):
        #     print('att_map_4_{}.shape\t{}'.format(i,att_map_4[i].shape))
        #     #torch.save(att_map_4[i], f'attmap4_{i}.pt')



        return [x, _c,att_map_1,att_map_2,att_map_3,att_map_4]
...
if __name__ == '__main__':
    ...
    model = MiT_B4(pretrained='segformer_MiT_B4_citys_best_model.pth',
                   img_size=(512, 512),
                   num_classes=19,
